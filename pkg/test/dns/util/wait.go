...
var (
	RecursiveNameservers = getNameservers(defaultResolvConf, defaultNameservers)
	DefaultDNSPort = "53"
)
...
func getNameservers(path string, defaults []string) []string {
	...
	for _, server := range config.Servers {
		...
		if _, _, err := net.SplitHostPort(server); err != nil {
			systemNameservers = append(systemNameservers, net.JoinHostPort(server, DefaultDNSPort))
		} else {
...
func checkDNSPropagation(fqdn, value string, nameservers []string,
	...
	for i, ans := range authoritativeNss {
		authoritativeNss[i] = net.JoinHostPort(ans, DefaultDNSPort)
	}
